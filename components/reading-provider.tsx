...
export function ReadingProvider({ children }: { children: React.ReactNode }) {
  ...
  // Hydrate from sessionStorage so choices persist across the flow.
  useEffect(() => {
    try {
  ...
  const setSetting = useCallback(
    <K extends keyof ReadingSettings>(key: K, value: ReadingSettings[K]) => {
      ...
      // Manual adjustment moves the user into a custom profile.
      setProfileId((prev) => (prev === "custom" ? prev : markCustom(prev, key)))
    },
    [],
  )
...
// Any manual typography change flips a preset into "custom" so the label stays honest.
function markCustom(prev: string, _key: keyof ReadingSettings) {
  return "custom"
}
